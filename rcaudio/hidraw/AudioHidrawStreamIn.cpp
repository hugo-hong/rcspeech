    ...
    : AudioStreamIn(owner)
{
    ...
    if (m_refNum++ == 0) {
        speech_in_open_stream(getDeviceInfo()->hidraw_index);
    }
}
...
uint32_t AudioHidrawStreamIn::getSampleRate()
{
    ...
    return speech_in_get_sample_rate((const struct audio_stream*)mStream);
}
...
size_t AudioHidrawStreamIn::getBufferSize()
{
    ...
    return speech_in_get_buffer_size((const struct audio_stream*)mStream);
}
...
status_t AudioHidrawStreamIn::standby()
{
    ...
    return speech_in_standby((struct audio_stream*)mStream);
}
...
ssize_t AudioHidrawStreamIn::read(void* buffer, size_t bytes)
{
    ...
    return speech_in_read(mStream, buffer, bytes);
}
